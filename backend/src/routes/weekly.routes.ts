...
import { getDailyTrend, getWeeklyTrend } from "../services/qualityQueries.js";
...
weeklyRouter.get("/weekly-trend", async (req, res, next) => {
  try {
    ...
    if (granularity === "daily") {
      const { dailyTrend, meta } = await getDailyTrend(filters);
      res.json({ weeklyTrend: dailyTrend, meta });
    } else {
